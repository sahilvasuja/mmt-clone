...
const flight = () => {
  ...
  async function showboardingpass() {
    // let encoded = encodeURIComponent(arr);
    // let decoded = decodeURIComponent(arr);
    // console.log(decoded + "23");
    const { data, error, status } = await supabase.from("my_trip").insert({
      from: arr.From,
      to: arr.To,
      ...
      <div className="bg-sky-900  ">
        <div className="   ">
          ...
          <div className="  flex flex-col justify-center ">
            <p className="mx-auto gap-y-5 font-light text-gray-500 dark:text-white">
              make my trip
            </p>
            <p className="mx-auto gap-y-5 font-light text-gray-500 dark:text-white">
              Trip to
            </p>
            <p className="mx-auto font-bold text-gray-500 text-4xl dark:text-white uppercase">
              {arr.To}
            </p>
          </div>
          <p className="m-4">{`${arr.flightname} | ${arr.flightnumber}`}</p>
          <div className="flex flex-row  sm:flex-row justify-between">
            <p className="mx-4 my-1 font-bold text-gray-500 dark:text-white text-xl lg:text-2xl">
              {arr.DepartureTime}
            </p>
            <p className="mx-4 my-1 font-bold text-gray-500 dark:text-white text-lg lg:text-lg">
              {arr.timerequired}
            </p>
            <p className="mx-4 my-1 font-bold text-gray-500 dark:text-white text-xl lg:text-2xl">
              {arr.ArrivalTime}
          ...
          <div className="flex flex-row  sm:flex-row justify-between">
            <p className="mx-4 my-1 font-light text-gray-500 dark:text-white">
              {arr.Date}
            </p>
            <p className="mx-3 my-1  text-2xl font-light text-gray-500 dark:text-white">
              ⏱
            </p>
            <p className="mx-4 my-1 font-light text-gray-500 dark:text-white">
              {arr.Date}
          ...
          <div className="flex flex-row  sm:flex-row justify-between">
            <p className="mx-4 my-1 font-bold text-lg uppercase text-gray-500 dark:text-white">
              {arr.From}
            </p>
            <p className=" mx-4 my-1 font-bold text-lg uppercase text-gray-500 dark:text-white">
              {arr.To}
              ...
              <div className="relative bg-white  rounded-lg shadow ">
                ...
                <div className="px-6 py-2  flex flex-row gap-x-28   ">
                  <p className="text-black flex flex-row gap-x-1   ">
                    ...
                    <span className="text-base leading-relaxed font-semibold text-black  ">
                      Cabin bag
                    </span>
                  </p>
          ...
          <div className="p-6 ">
            <p className=" rounded-lg font-light text-gray-500 dark:text-gray-400 bg-white p-8 ">
              By clicking on the Continue button below to proceed with the
              booking, I confirm that I have read and I accept the{" "}
              <a
                href="#"
                className="font-medium text-blue-600 dark:text-blue-500 dark:hover:text-blue-600 hover:text-blue-700 hover:no-underline"
              >
                Fare Rules
              </a>{" "}
              ,the{" "}
        ...
        <div className="bg-black p-5 flex flex-row justify-between ">
          ...
          <button
            type="submit"
            onClick={showboardingpass}
            className="bg-sky-600 rounded-xl px-4 py-2 z-50"
          >
            {" "}
            CONTINUE
          </button>
